...
assert.strictEqual(output.decision_defaults.readiness_state, 'not_pit_ready');
assert.strictEqual(output.decision_defaults.manual_review, true);
assert.strictEqual(output.decision_defaults.positive_imputation, false);
assert.strictEqual(output.decision_defaults.wave_c_source_timestamp_precision, 'listing_only');
assert.strictEqual(output.decision_defaults.wave_c_parser_version, 'institutional-accumulation-material-information-wave-c-v1');
assert.strictEqual(output.decision_defaults.wave_c_version_safety, 'historical_timing_safe_value_version_unproven');
assert.strictEqual(output.decision_defaults.wave_c_pit_known_at, null);
...
for (const required of ['wave_a_known_after_t0', 'wave_c_historical_value_version_unproven', 'wave_c_pit_known_at_unproven']) {
  assert.ok(output.decision_defaults.reason_codes.includes(required));
}
