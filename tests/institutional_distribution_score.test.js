...
const historyRoot = path.join(root, 'data_tdcc_shareholding', 'history', '2449');
...
test('official TDCC historical replay preserves milestones but never invents action timing', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'institutional-distribution-history-'));
  const output = path.join(tmp, '2449-history.json');
  execFileSync(process.execPath, [script,
    '--stock', '2449',
    '--start', '2026-04-01',
    '--end', '2026-07-31',
    '--tdcc-mode', 'historical',
    '--tdcc-history-root', historyRoot,
    '--output', output,
  ], { cwd: root, stdio: 'pipe' });

  const payload = JSON.parse(fs.readFileSync(output, 'utf8'));
  assert.equal(payload.tdcc_input.mode, 'historical');
  assert.equal(payload.research_only, true);
  assert.equal(payload.production_safe, false);
  assert.equal(payload.no_lookahead_contract.action_gate, null);
  assert.ok(payload.tdcc_input.observations >= 10);

  const byDate = new Map(payload.timeline.map((x) => [x.observed_date, x]));
  assert.ok(rank[byDate.get('2026-04-17').level] >= rank.yellow, 'official history: 2026-04-17 should be at least Yellow');
  assert.ok(rank[byDate.get('2026-04-24').level] >= rank.orange, 'official history: 2026-04-24 should be at least Orange');
  assert.equal(byDate.get('2026-05-15').level, 'red', 'official history: 2026-05-15 should be Red');
  for (const row of payload.timeline) assert.equal(row.action_eligible_after, null, `${row.observed_date} historical replay must not invent action eligibility`);
});
