...
test('material information classifier separates conference and preliminary earnings', () => {
...
test('monthly revenue normalizer preserves official date precision and metrics', () => {
  ...
  assert.equal(event.timestamp_precision, 'date');
  assert.equal(event.published_date, '2026-08-10');
...
test('material normalizer uses official second-level timestamp', () => {
...
test('duplicate events prefer better availability confidence', () => {
  const fallback = finalizeEvent({ stock_id: '2330', event_type: EVENT_TYPES.PRELIMINARY_EARNINGS, published_date: '2026-07-16', timestamp_precision: 'date', availability_confidence: 'fallback_deadline', event_id: 'same' }, ['2026-07-16','2026-07-17']);
  const exact = finalizeEvent({ stock_id: '2330', event_type: EVENT_TYPES.PRELIMINARY_EARNINGS, published_at: '2026-07-16T14:00:00+08:00', timestamp_precision: 'minute', availability_confidence: 'official_timestamp', event_id: 'same' }, ['2026-07-16','2026-07-17']);
  const result = dedupeEvents([fallback, exact]);
