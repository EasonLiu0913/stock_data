...
dashboard 聚合層也會計算第一版 `多日盤整+趨勢轉強` 策略標籤。這個標籤不直接改變個股預測分數，只用於策略分類與後續覆盤。判斷方式分成「盤整分數」與「轉強分數」：

盤整分數來源：

- 20 日區間收斂。
- 10 日區間收斂。
- SMA5 / SMA20 / SMA60 均線糾結。
- SMA20 近 5 日斜率接近平坦。
- 近 5 日波動低於近 20 日平均波動。
- 近 5 日量能低於近 20 日均量。
- RSI14 位於中性整理區。
...
- 轉強分數至少 5。
- 必須具備 10 日或 20 日區間收斂。
- 必須具備價格轉強訊號：收盤站上 SMA20 或接近 20 日高點。
- 必須具備動能轉強訊號：MACD、KD 或 RSI 任一項轉強。
- `direction_score >= 0`，且最終方向不得為偏空。
- `data_completeness >= 80`，個股風險不得為高風險。
- RSI14 不得過熱，且收盤距 SMA20 不得過遠。

命中股票會在 `summary.json` 保留 `consolidation_strength` 明細，包含盤整分數、轉強分數、原因標籤與主要量化指標，供策略分類與事後覆盤使用。
