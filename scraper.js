...
(async () => {
    ...
    if (fs.existsSync(cookiesPath)) {
        ...
        console.log('⚠️  No cookies.json found. Run extract_cookies.js in browser console first.');
        console.log('   See extract_cookies.js for instructions.');
    }
    ...
    try {
        ...
        await page.waitForSelector('table#netBuyRank tbody tr', { timeout: 10000 });

        // Wait for data to populate (browser subagent used 5 seconds successfully)
        ...
        if (data.length > 0) {
            ...
            const csvContent = [
                headers.join(','),
                ...data.map(row => {
                    // Ensure we only take first 7 columns and escape quotes if needed
