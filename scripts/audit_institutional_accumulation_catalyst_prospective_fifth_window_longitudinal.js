...
const AUDIT_ID = 'institutional-accumulation-catalyst-prospective-fifth-window-longitudinal-audit-v1';
const OUTPUT_RELATIVE = 'data_research/institutional-flow/institutional-accumulation-catalyst-prospective-fifth-window-longitudinal-audit-v1.json';
...
function taipeiDate(ms) { return new Date(ms + 8 * 60 * 60 * 1000).toISOString().slice(0, 10); }
...
function buildFifthWindowLongitudinalAudit(repoRoot, options = {}) {
  ...
  for (const stock of EXPECTED_STOCKS) {
    ...
    for (const sourceInterface of EXPECTED_INTERFACES) {
      ...
      chains.push({
        ...
        windows: [w1,w2,w3,w4,w5].map((x,index) => ({
          window:index+1, source_path:x.source_path, collected_at:x.collected_at,
          asia_taipei_date:taipeiDate(x.collected_ms), immutable_snapshot_id:x.immutable_snapshot_id,
          response_sha256:x.response_sha256, response_bytes:x.response_bytes,
        })),
        window_4_to_5_elapsed_milliseconds: w5.collected_ms - w4.collected_ms,
      });
    }
  }
  const latestFourthDate=taipeiDate(latestFourthMs);
  const earliestFifthDate=taipeiDate(earliestFifthMs);
  const elapsedMs=earliestFifthMs-latestFourthMs;
  if (earliestFifthDate <= latestFourthDate) throw new Error(`cross_day_requirement_failed:${latestFourthDate}:${earliestFifthDate}`);
  ...
  return {
    ...
    fifth_window_gate:{
      ...
      earliest_fifth_collected_at:new Date(earliestFifthMs).toISOString(),
      earliest_fifth_asia_taipei_date:earliestFifthDate,
...
function main(){
  const audit=buildFifthWindowLongitudinalAudit(process.cwd());
...
module.exports={AUDIT_ID,OUTPUT_RELATIVE,EXPECTED_STOCKS,EXPECTED_INTERFACES,MIN_ELAPSED_MS,parseUtc,taipeiDate,buildFifthWindowLongitudinalAudit,serializeAudit};
