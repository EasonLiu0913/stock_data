...
function outputPathForDate(dateCompact) {
    return path.join(OUTPUT_DIR, `${dateCompact}_twse_foreign_investors.json`);
}
...
(async () => {
    try {
        ...
        console.log('🚀 TWSE foreign investors history crawl');
        ...
        refreshFilesJson();
        console.log('📁 Refreshed data_twse_foreign_investors/files.json');
    } catch (error) {
        console.error(`❌ Failed to crawl TWSE foreign investors history: ${error.message}`);
        process.exit(1);
    }
})();
