...
const API_URL = 'https://www.twse.com.tw/rwd/zh/fund/T86';
const OUTPUT_DIR = path.join(__dirname, '../data_twse_institutional_investors');
...
function outputPathForDate(dateCompact) {
    return path.join(OUTPUT_DIR, `${dateCompact}_twse_institutional_investors.json`);
}
...
(async () => {
    try {
        ...
        console.log('🚀 TWSE institutional investors history crawl');
        ...
        refreshFilesJson();
        console.log('📁 Refreshed data_twse_institutional_investors/files.json');
    } catch (error) {
        console.error(`❌ Failed to crawl TWSE institutional investors history: ${error.message}`);
        process.exit(1);
    }
})();
