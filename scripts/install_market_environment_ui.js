...
const REBOUND_SCRIPT = 'prediction-dashboard-rebound-enhancement.js?v=1';
...
function installMarketEnvironment(html) {
...
function installReboundReadiness(html) {
  let output = html;
  if (!output.includes('id="oversoldBetaReboundBanner"')) {
    const environmentSection = '<section class="environment-banner" id="marketEnvironmentBanner"><div class="environment-title">市場環境載入中…</div></section>';
    output = replaceOnce(
      output,
      environmentSection,
      `${environmentSection}<section class="rebound-readiness-card readiness-unavailable" id="oversoldBetaReboundBanner"><div class="readiness-title">跌深反彈準備度載入中…</div></section>`,
      'market environment banner',
    );
  }
  if (!output.includes(REBOUND_SCRIPT)) {
    output = replaceOnce(
      output,
      '</body>',
      `  <script src="${REBOUND_SCRIPT}"></script>\n</body>`,
      'body close',
    );
  }
  return output;
...
function main() {
  ...
  const original = html;
  html = installMarketEnvironment(html);
  html = installReboundReadiness(html);
  html = html
  ...
  if (html === original) {
    console.log('Market environment and rebound readiness UI already installed');
    return;
  }
  fs.writeFileSync(FILE, html, 'utf8');
  console.log(`Installed market environment and rebound readiness UI in ${path.relative(ROOT, FILE)}`);
}
...
module.exports = {
  replaceOnce,
  installMarketEnvironment,
  installReboundReadiness,
  main,
};
