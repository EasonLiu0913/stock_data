...
function main() {
  ...
  const keys = missing.map((row) => `${row.stock}|${row.t0}`);
  if (new Set(keys).size !== 33) throw new Error('Frozen source_missing identities contain duplicates');
  ...
  const decisions = missing.map((row) => {
    ...
    const waveAKnownAfterT0 = waveA.pit_known_at == null || parseYYYYMMDD(waveA.pit_known_at) > parseYYYYMMDD(row.t0);
    const waveCPitUnproven = waveC.pit_known_at == null;
    const waveCVersionUnproven = String(waveC.version_safety || '').includes('unproven');
    const ready = !waveAKnownAfterT0 && !waveCPitUnproven && !waveCVersionUnproven;

    if (ready) {
      throw new Error(`Fail-safe violation: ${row.stock}|${row.t0} unexpectedly became readiness-positive`);
    }
    ...
    return {
      ...
      wave_a_source_meta_path: path.relative(ROOT, WAVE_A_META).replaceAll(path.sep, '/'),
      ...
      wave_c_source_timestamp_precision: waveC.source_timestamp_precision ?? null,
      wave_c_parser_version: waveC.parser_version ?? null,
      wave_c_version_safety: waveC.version_safety ?? null,
      wave_c_pit_known_at: waveC.pit_known_at ?? null,
      reason_codes: [
        ...(waveAKnownAfterT0 ? ['wave_a_known_after_t0'] : []),
        ...(waveCVersionUnproven ? ['wave_c_historical_value_version_unproven'] : []),
        ...(waveCPitUnproven ? ['wave_c_pit_known_at_unproven'] : []),
      ],
    };
  });
  ...
  const output = {
    ...
    ready_identity_count: decisions.filter((row) => row.readiness_state === 'ready').length,
    not_ready_identity_count: decisions.filter((row) => row.readiness_state !== 'ready').length,
  ...
  console.log(JSON.stringify({
    ...
    identities: decisions.length,
    ready: output.ready_identity_count,
    not_ready: output.not_ready_identity_count,
    network_requests: 0,
  }));
}
