...
const { DAILY_GAINERS_AI_CONTRACT: CONTRACT, isLatestFacts, isLatestAi, isLatestPublished } = require('./lib/daily_gainers_ai_contract');
function readJson(file) { return JSON.parse(fs.readFileSync(file, 'utf8')); }
function usage() { throw new Error('Usage: node scripts/daily_gainers_ai_contract_cli.js <print|facts-latest|ai-latest|published-latest|assert-facts|assert-ai|assert-published> [file]'); }
const command = process.argv[2]; const file = process.argv[3];
try {
  ...
  if (command === 'published-latest') process.exit(isLatestPublished(payload) ? 0 : 1);
  ...
  if (command === 'assert-published') {
    if (!isLatestPublished(payload)) throw new Error(`Published analysis is not latest: expected schema=${CONTRACT.published.schema_version}, methodology=${CONTRACT.published.methodology_version}`);
    process.stdout.write(`${CONTRACT.published.methodology_version}\n`); process.exit(0);
  }
  usage();
