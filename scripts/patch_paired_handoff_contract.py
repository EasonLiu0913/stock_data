...
"""
if old not in text:
    raise SystemExit("required contents marker missing")
text = text.replace(old, new, 1)

marker = "### Copy-paste next-round prompt\n"
if marker not in text:
    raise SystemExit("copy-paste heading missing")
paired = """### Paired implementation + closeout prompts
...
"""
text = text.replace(marker, paired + marker, 1)
text = text.replace(
...
handoff = Path("data_research/institutional-flow/institutional-withdrawal-validation-handoff.md")
h = handoff.read_text()
anchor = "# 1. Frozen methodology / leakage guardrails\n"
if anchor not in h:
    raise SystemExit("handoff anchor missing")
flow = """# Handoff execution loop
...
"""
h = h.replace(anchor, flow + anchor, 1)

prompt_heading = "# 9. Ready-to-paste fresh conversation prompt\n"
if prompt_heading not in h:
    raise SystemExit("prompt heading missing")
h = h.replace(
    prompt_heading,
