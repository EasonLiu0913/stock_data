...
# The plan job already verified that a stored CNN payload with a historical
# series is available. Prefer it in matrix jobs so a persistent CNN 418 does
# not cause every batch to repeat failed network requests.
args=(
  --dates "$BATCH_DATES"
...
NODE
  ...
  if [ "$index" -lt "$((${#dates[@]} - 1))" ]; then
    min="$MIN_DATE_PAUSE_SECONDS"
    max="$MAX_DATE_PAUSE_SECONDS"
    [ "$max" -lt "$min" ] && max="$min"
