...
const AUDIT_ID = 'institutional-accumulation-catalyst-prospective-fourth-window-longitudinal-audit-v1';
const OUTPUT_RELATIVE = 'data_research/institutional-flow/institutional-accumulation-catalyst-prospective-fourth-window-longitudinal-audit-v1.json';
...
function buildFourthWindowLongitudinalAudit(repoRoot, options = {}) {
  ...
  for (const stock of EXPECTED_STOCKS) {
    ...
    for (const sourceInterface of EXPECTED_INTERFACES) {
      ...
      chains.push({
        ...
        windows: [w1, w2, w3, w4].map((x, index) => ({
          window: index + 1,
        ...
        window_3_to_4_elapsed_milliseconds: w4.collected_ms - w3.collected_ms,
      });
    }
  }

  const latestThirdDate = taipeiDate(latestThirdMs);
  const earliestFourthDate = taipeiDate(earliestFourthMs);
  const elapsedMs = earliestFourthMs - latestThirdMs;
  if (earliestFourthDate <= latestThirdDate) throw new Error(`cross_day_requirement_failed:${latestThirdDate}:${earliestFourthDate}`);
  ...
  return {
    ...
    fourth_window_gate: {
      ...
      earliest_fourth_collected_at: new Date(earliestFourthMs).toISOString(),
      earliest_fourth_asia_taipei_date: earliestFourthDate,
...
function main() {
  const audit = buildFourthWindowLongitudinalAudit(process.cwd());
...
module.exports = { AUDIT_ID, OUTPUT_RELATIVE, EXPECTED_STOCKS, EXPECTED_INTERFACES, MIN_ELAPSED_MS, parseUtc, taipeiDate, buildFourthWindowLongitudinalAudit, serializeAudit };
