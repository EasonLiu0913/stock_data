...
function buildCrossDayAudit(repoRoot, options = {}) {
  ...
  for (const stock of EXPECTED_STOCKS) {
    for (const sourceInterface of EXPECTED_INTERFACES) {
      ...
      const [w1, w2, w3] = frozenOccurrences;
  ...
  return {
    ...
    chain_count: 6,
    stock_count: 3,
    unique_immutable_snapshot_count: new Set(chains.flatMap(chain => chain.windows.map(window => window.immutable_snapshot_id))).size,
    unique_response_sha256_count: new Set(chains.flatMap(chain => chain.windows.map(window => window.response_sha256))).size,
    collection_time_range: {
      first: chains.flatMap(chain => chain.windows.map(window => window.collected_at)).sort()[0],
      last: chains.flatMap(chain => chain.windows.map(window => window.collected_at)).sort().at(-1),
    },
    cross_day_gate: {
