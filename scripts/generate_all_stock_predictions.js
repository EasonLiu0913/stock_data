...
function main() {
  ...
  for (const [code, meta] of Object.entries(stocks)) {
    ...
    if(Number.isFinite(marketRiskScore) && marketRiskScore >= 65)risk+=2; else if(Number.isFinite(marketRiskScore) && marketRiskScore >= 40)risk+=1;
    if(Number.isFinite(adrRiskScore) && adrRiskScore >= 70)risk+=1;
    if(Number.isFinite(oilRiskScore) && oilRiskScore >= 70)risk+=1;
