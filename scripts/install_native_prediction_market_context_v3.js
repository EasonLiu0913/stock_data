...
function patchResolver() {
  ...
  const filtered = read(relative)
    .split('\n')
    .filter((line) => !line.includes('MARKET_CONTEXT_PRELOAD'))
    .filter((line) => !line.includes('const preloadOption'))
    .filter((line) => !line.includes('const existingNodeOptions'))
    .filter((line) => !line.includes('`NODE_OPTIONS='))
    .join('\n');
  if (filtered.includes('MARKET_CONTEXT_PRELOAD') || filtered.includes('`NODE_OPTIONS=')) {
    throw new Error('Resolver still contains preload or emitted NODE_OPTIONS');
  }
  write(relative, filtered.replace(/\n{3,}/g, '\n\n'));
}
...
function main() {
  ...
  patchTests();
  cleanup();
  console.log(JSON.stringify({ migrated: true, mode: 'native_prediction_market_context_v3' }));
}
