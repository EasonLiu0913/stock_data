...
function build() {
  ...
  for (const stock of stocks) {
    ...
    for (let i = 0; i < series.length; i += 1) {
      ...
      const distributionDays5 = trailing5Complete ? trailing5.filter((x) => {
        const idx = tradingDays.indexOf(x.date);
        const prior = idx >= 20 ? series.slice(idx - 20, idx) : [];
        if (prior.length !== 20 || !prior.every(isPresent)) return false;
        ...
        return Number.isFinite(vr) && Number.isFinite(r1) && vr >= 1.5 && r1 <= -1;
      }).length : null;
      const distributionDays10 = trailing10Complete ? trailing10.filter((x) => {
        const idx = tradingDays.indexOf(x.date);
        const prior = idx >= 20 ? series.slice(idx - 20, idx) : [];
        if (prior.length !== 20 || !prior.every(isPresent)) return false;
        const base = mean(prior.map((y) => y.volume));
      ...
      const absorptionDays10 = trailing10Complete ? trailing10.filter((x) => {
        const idx = tradingDays.indexOf(x.date);
        const prior = idx >= 20 ? series.slice(idx - 20, idx) : [];
        if (prior.length !== 20 || !prior.every(isPresent)) return false;
        const base = mean(prior.map((y) => y.volume));
        ...
        const r1 = priorDay ? pct(x.close, priorDay.close) : null;
        const vr = base > 0 ? x.volume / base : null;
        return Number.isFinite(vr) && Number.isFinite(r1) && vr >= 1.5 && Math.abs(r1) <= 1;
      }).length : null;
      ...
      rows.push({
        ...
        distribution_days_5d: distributionDays5,
        distribution_days_10d: distributionDays10,
        absorption_days_10d: absorptionDays10,
        price_volume_confirm: Boolean((Number.isFinite(distributionDays10) && distributionDays10 >= 2) || (Number.isFinite(volumeRatio) && volumeRatio >= 1.5 && Number.isFinite(return5d) && return5d < 0)),
      });
    }
  }
  ...
  const payload = {
    schema_version: 2,
    methodology: 'institutional-withdrawal-v5-price-volume-features-v2-gap-preserving',
    research_only: true,
    range: { start, end }, universe: stocks,
    source: 'Fubon SMA daily OHLCV snapshots',
    no_lookahead: 'Volume baseline uses the previous 20 calendar-listed trading sessions and excludes current. Missing rows invalidate every affected rolling window; they are never skipped, compressed, or imputed.',
    ...
    source_row_gaps: sourceRowGaps,
    coverage, rows,
    generated_at: new Date().toISOString(),
  };
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.writeFileSync(output, `${JSON.stringify(payload, null, 2)}\n`);
  console.log(JSON.stringify({ counts: payload.counts, coverage, source_row_gaps: sourceRowGaps }, null, 2));
  return payload;
}
