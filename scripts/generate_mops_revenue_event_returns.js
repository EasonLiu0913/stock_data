...
function buildEvent(row, revenueMonth, marketRows) {
  ...
  if (!timing.usable) {
    return { ...base, effective_trading_date: null, base_trading_date: null, returns: {}, evaluation_status: 'excluded_missing_original_event_time' };
  }
  const calculated = calculateEventReturns(row.stock_code, timing.observed_date, marketRows);
  return { ...base, ...calculated, evaluation_status: calculated.effective_trading_date ? 'eligible' : 'pending_next_trading_day' };
}
...
module.exports = { HORIZONS, buildTradingWindow, classifyObservedTiming, generateMonth, pctReturn };
