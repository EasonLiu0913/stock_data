...
function validatePayload(payload) {
    const requiredArrays = ['fields', 'data'];
...
async function fetchTwseForeignInvestorsOnce(dateStr = '') {
    const params = new URLSearchParams({ selectType: 'ALL', response: 'json', _: String(Date.now()) });
...
(async () => {
    try {
        ...
        const outputFile = `${payload.date}_twse_institutional_investors.json`;
        ...
        console.error(`❌ Failed to crawl TWSE institutional investors data: ${error.message}`);
        process.exit(1);
    }
})();
