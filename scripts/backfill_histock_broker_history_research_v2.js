...
function pageDiagnostics(html, response, date) {
  ...
  return {
    ...
    date_visible: [compact, date, date.replaceAll('-', '/')].some((token) => html.includes(token) || pageText.includes(token)),
    broker_keywords_visible: /券商|買進|賣出|買超|賣超/.test(pageText),
...
async function fetchDateOnce(date) {
  ...
  const diagnostics = pageDiagnostics(html, response, date);
  if (!response.ok) {
  ...
  return { url, bytes: diagnostics.response_bytes, records, diagnostics };
}
...
async function fetchDate(date) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      if (!canRetry) break;
      const backoffMs = Math.min(30000, Math.max(4000, delayMs * (2 ** attempt))) + randomJitter(jitterMs);
      console.log(`retry ${attempt + 1}/${maxRetries} after ${backoffMs}ms (${error.message}; bytes=${error.diagnostics?.response_bytes ?? 'n/a'}; rows=${error.diagnostics?.table_rows ?? 'n/a'})`);
...
async function main() {
  ...
  const complete = !aborted && unresolved.length === 0 && days.length === requested.length;
  const analysis = {
    schema_version: 2,
    ...
    counts: {
      ...
      remote_requests_attempted: requestedRemote,
