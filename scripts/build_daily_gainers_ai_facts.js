...
function institutionalActor(source,code,legacyNet) {
  ...
  return {net_shares:row.net_shares,net_lots:lots,record_status:row.net_shares===0?'zero_net':'reported',requires_external_verification:row.net_shares===0,source_file:source.file};
}
...
function main() {
  ...
  const stocks=list.stocks.map(stock=>{
    ...
    institutional.verification_required=Object.values(institutional).some(v=>v&&typeof v==='object'&&v.requires_external_verification===true);
  ...
  const payload={schema_version:2,methodology_version:'daily-gainers-ai-facts-v2',target_date:date,generated_at:new Date().toISOString(),purpose:'deterministic_fact_package_for_ai_synthesis',ai_must_not_invent_missing_values:true,institutional_status_semantics:{reported:'official source contains the stock row and net is non-zero',zero_net:'official source contains the stock row and net equals zero',no_record:'official source is valid but contains no row for this stock',unavailable:'official source file is missing, empty, malformed, or not OK'},source_files:{list:path.relative(ROOT,listFile),catalyst_analysis:path.relative(ROOT,causeFile),flow_analysis:path.relative(ROOT,flowFile),margin:path.relative(ROOT,marginFile),broker_details:path.relative(ROOT,brokerFile),mi_index:path.relative(ROOT,miFile),foreign:institutionalSources.foreign.file,trust:institutionalSources.trust.file,dealer:institutionalSources.dealer.file},institutional_source_status:{foreign:{status:institutionalSources.foreign.status,row_count:institutionalSources.foreign.row_count},trust:{status:institutionalSources.trust.status,row_count:institutionalSources.trust.row_count},dealer:{status:institutionalSources.dealer.status,row_count:institutionalSources.dealer.row_count}},stock_count:stocks.length,market_context:marketContext(mi),stocks};
  fs.mkdirSync(FACT_DIR,{recursive:true}); const out=path.join(FACT_DIR,`${date}.json`); fs.writeFileSync(out,`${JSON.stringify(payload,null,2)}\n`);
  console.log(JSON.stringify({output:path.relative(ROOT,out),stock_count:stocks.length,verification_required:stocks.filter(s=>s.institutional.verification_required).length},null,2));
}
